  ...
  return (
    <div className="pt-32 pb-20">
      <div className="container mx-auto px-6">
        {/* Header Section */}
        <div className="mb-16">
          <Link to="/services" className="inline-flex items-center gap-2 text-xs font-bold tracking-widest text-gray-400 hover:text-[#BBA899] transition-colors mb-8 uppercase">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-24 mb-32">
          ...
          <div className="space-y-12">
            <div className="bg-white p-10 rounded-sm shadow-sm border border-gray-100">
              ...
              <Link to="/contact" className="mt-12 w-full bg-[#191919] text-white py-5 flex items-center justify-center gap-3 hover:bg-[#BBA899] transition-all uppercase tracking-[0.3em] font-bold text-[10px]">
            ...
            {otherServices.map(s => (
              <Link key={s.id} to={`/services/${s.id}`} className="group block">
                <div className="aspect-video overflow-hidden mb-4 rounded-sm">
                  <img src={s.image} alt={s.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500 group-hover:scale-105" />
                </div>
